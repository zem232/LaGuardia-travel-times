...
if line == 'M60':
    BUSID = BUS[' BusID'].unique()
    for x in BUSID:
        if ('HOYT AV/31 ST' in BUS[BUS[' BusID']==x]['Stop Name'].unique()):
            a='continue'
        else:
            print('delete +'+ x)
            BUS = BUS[BUS[' BusID'] != x]
        ...
        try:
            a.append(BUS[(BUS[' BusID'] == x) & 
                 ...
                 ((BUS['Stop Status'] == 'at stop') | 
                  ...
                  (BUS['Stop Status'] == '< 1 stop away'))].tail(1).reset_index()['Date'][0])
        except IndexError:
            a.append(BUS[(BUS[' BusID'] == x) & 
                         (BUS['Stop Name'] == LGASTOP)
...
csvname = 'M60_Hoyt31_travel_time.csv'
