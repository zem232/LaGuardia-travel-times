...
## Isolate buses going in direction of LGA
BUS = BUS[BUS['Direction'] == 1]
...
if line == 'Q48':
    for x in BUSID:
        if ('ROOSEVELT AV/MAIN ST' in BUS[BUS[' BusID']==x]['Stop Name'].unique()) or ('ROOSEVELT AV/COLLEGE PT BL' in BUS[BUS[' BusID']==x]['Stop Name'].unique()):
            a='continue'
        else:
            print('delete +'+ x)
            BUS = BUS[BUS[' BusID'] != x]
    BUSID = BUS[' BusID'].unique()
    for x in BUSID:
        if LGASTOP in BUS[BUS[' BusID']==x]['Stop Name'].unique():
            a='continue'
        else:
            print('delete +'+ x)
            BUS = BUS[BUS[' BusID'] != x]
    ...
    b = []
    for x in BUSID:
## Use head since this bus runs on a loop, so you want to extract the first value.
        try:
        ...
        b.append(BUS[(BUS[' BusID'] == x) & 
                     ((BUS['Stop Name'] == 'ROOSEVELT AV/MAIN ST') |
                      (BUS['Stop Name'] == 'ROOSEVELT AV/COLLEGE PT BL')) &
                         ((BUS['Stop Status'] == 'at stop') | 
                          (BUS['Stop Status'] == 'approaching') | 
    ...
    csvname = name +'_Flushing_travel_time.csv'
